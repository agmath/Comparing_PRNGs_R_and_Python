...
A repository for code to accompany the paper titled *Are we rolling the same dice? PRNG Consistency Across R and Python* by Mata-Toledo, Dumnich, Ryan, and Gilbert. Functions utilized and referenced throughout the paper are contained in the `CurrentPaperCode` folder. Any script names beginning with `generate_*` are helper functions referenced by the other scripts in the folder. These helper functions are used to generate the random sequences of values.
